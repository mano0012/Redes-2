//ALunos: Marcos Felipe e Edimilton
...
void simulacao(uint32_t qtd_nos, uint32_t area, int rts, FILE **file_tx_rcv, FILE **file_through, int matriz_posicao_nos[][2]);
...
int main(int argc, char **argv){
  ...
  FILE  *file_tx_rcv, *through;
  file_tx_rcv = fopen("Razao", "wt");
  ...
  // Teste sem RTS/CTS
   	simulacao(qtd_nos,area,1, &file_tx_rcv, &through,matriz_posicao_nos);
  // Teste com RTS/CTS
    simulacao(qtd_nos,area,0, &file_tx_rcv, &through,matriz_posicao_nos);
}
...
void simulacao(uint32_t qtd_nos, uint32_t area, int rts, FILE **file_tx_rcv, FILE **file_through, int matriz_posicao_nos[][2]){
  ...
  if(rts == 1){
   ...
   float distancia_nos[qtd_nos][qtd_nos];
  ...
  //  Setando a taxa de transmissao:
   ...
   uint32_t fluxo = 0, sender, receiver = 0;
  ...
  //  Agendando o stop:
   ...
   fprintf(*file_through, "%lf\n", vazao / qtd_fluxos); 
   fprintf(*file_tx_rcv, "%lf\n", Razao / qtd_fluxos);
   Simulator::Destroy ();
